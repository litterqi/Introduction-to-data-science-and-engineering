# 探究NBA球员选秀顺位与成才之间的关系
## 前言
![image](https://github.com/litterqi/Introduction-to-data-science-and-engineering/assets/123362884/1161ea4a-044c-4299-8c15-092b89acab07)
...
对数据集中满足比赛场数大于250场的球员绘制3d散点图，其中x轴表示年份（"year"列），y轴表示选秀顺位（"overall_pick"列），z轴表示得分（"points"列）。可以看出总体上球员选秀顺位越高，生涯总得分也相应越高。这一定程度上体现其职业生涯的长度与稳定性。
...
## 总结
在本次研究中，我们对NBA1989-2021年选秀球员进行了数据处理和分析。其中，我们应用了数据科学中常用的研究方法，如数据分析，可视化，机器学习等，通过统计数据，绘制相关图表，构建预测模型等手段对球员的各项数据进行了研究。这不仅增强了我们对球员成才与否和其选秀顺位之间联系的整体认识，也为球队在发掘，考察新秀球员时如何进行选择提供了可靠的依据。

与此同时，在研究过程中对异常数据值的分析，也为我们提供启示：即便选秀顺位不高，依然有球员得到20+的场均得分，超过了很多顺位在其之上的球员。谁强谁弱，孰优孰劣，本身就是一个不断变化的过程。篮球如此，人生亦然。一时输赢，不代表最终的结局，重要的是不怕困难，超越自己。
